...
export default function Orb({
  ...
  const frag = /* glsl */ `
    ...
    varying vec2 vUv;
    // ...existing shader helper functions...
    ...
    vec4 mainImage(vec2 fragCoord){
      ...
      // Warp only if both hover and hoverIntensity present (prevents sudden pop when hoverIntensity -> 0)
      float eff = hover * hoverIntensity;
      if(eff > 0.0001){
        uv.x += eff * 0.1 * sin(uv.y*10.0 + iTime);
        uv.y += eff * 0.1 * sin(uv.x*10.0 + iTime);
      }
      return draw(uv);
    }
    void main(){
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      program.uniforms.hue.value += (hueRef.current - program.uniforms.hue.value) * 0.12;
      program.uniforms.hoverIntensity.value +=
        (hoverIntensityRef.current - program.uniforms.hoverIntensity.value) * 0.2;

      // Hover easing
      const desiredHover = forceHoverStateRef.current ? 1 : targetHover;
      program.uniforms.hover.value += (desiredHover - program.uniforms.hover.value) * 0.1;

      // Rotation only when "active"
      if (rotateOnHoverRef.current && program.uniforms.hover.value > 0.5 && hoverIntensityRef.current > 0.05) {
