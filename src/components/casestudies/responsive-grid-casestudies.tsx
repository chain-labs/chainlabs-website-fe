...
const ResponsiveGridCasestudies = () => {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("openCaseStudy", handler as EventListener);
  }, [caseStudies]);
  ...
  useEffect(() => {
    ...
    if (
      prev &&
      (!active || (typeof active === "object" && active.id !== prev.id)) &&
    ...
    return () => {
      ...
      if (timerStartRef.current && prevCaseStudyRef.current) {
        const elapsedMs = Date.now() - timerStartRef.current;
        const seconds = Math.floor(elapsedMs / 1000);
        if (seconds > 0) addCaseStudyTime(prevCaseStudyRef.current.id, seconds);
      }
    };
    ...
    >
      ...
      >
        ...
        <p className="mt-4 text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
          Snapshot outcomes: faster operations, new revenue streams, and
          elevated customer experiences delivered through applied AI.
        </p>
      </motion.div>
