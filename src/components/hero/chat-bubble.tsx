...
const ChatBubble = React.memo(
  ({ message, isLatest }: { message: ChatMessage; isLatest: boolean }) => {
    const isUser = message.role === "user";
    const hasAnimated = React.useRef(false);
    const [shouldAnimate, setShouldAnimate] = React.useState(false);

    // Treat non-user React elements as rich content and render directly
    const isAssistant = !isUser;
    const isRichContent = isAssistant && React.isValidElement(message.message);

    // Skip typing animation for rich (non-string) assistant content
    useEffect(() => {
      if (isRichContent) {
