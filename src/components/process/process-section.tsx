...
export const ProcessSection = () => {
  ...
  const addProcessSectionTime = useGlobalStore((s) => s.addProcessSectionTime);
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        const newStepId = visible
          ? (visible.target as HTMLElement).dataset.processStepId || null
          : null;
    ...
    return () => {
      ...
      if (prevStepRef.current && timerStartRef.current) {
        const elapsedMs = Date.now() - timerStartRef.current;
        const seconds = Math.floor(elapsedMs / 1000);
        if (seconds > 0) addProcessSectionTime(prevStepRef.current, seconds);
      }
    };
    ...
    >
      ...
      >
        ...
        <p className="mt-4 max-w-2xl mx-auto text-base sm:text-lg text-muted-foreground leading-relaxed">
          A transparent 6‑step path from opportunity discovery to scalable
          deployment— engineered to de‑risk your AI investment and accelerate
          time‑to‑value.
        </p>
      </motion.div>
