...
import { Bot, Minus, SendHorizontal, Calendar, Mic, MicOff, User } from "lucide-react";
...
const AIChatBubble = () => {
    ...
    const showBookCall = useGlobalStore().personalised?.personalisation.call_unlocked;
    ...
    const { showPersonalized, isRecording, toggleRecording, stopRecording } = useUI();
    ...
    const autoResize = () => {
        ...
        const maxPx = isMobile ? 0.35 * window.innerHeight : 0.4 * window.innerHeight;
    ...
    useEffect(() => {
        ...
        if (!listening && wasListeningRef.current) {
            ...
            if (spokenText) {
                setInputValue(inputValue ? `${inputValue} ${spokenText}` : spokenText);
            }
    ...
    }, [listening, transcript, setInputValue, setVoiceInputValue, resetTranscript]);
    ...
    const handleSendMessage = async (e: React.FormEvent<HTMLFormElement> | string) => {
        ...
        const content = typeof e === "string" ? e : (inputValue + voiceInputValue).trim();
        if (!content) return;
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        {hasMessages ? "Continue our conversation" : "Ready to help"}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
                            {messages.map((message, index) => (
                                <MessageBubble
                                    ...
                                    message={message}
                                    isLatest={index === messages.length - 1 && isThisLatestAssistantMessage}
                                />
                            ))}
                            ...
                            {messages.length <= 3 && !isThinking && (
                                <div className="pt-2">
                                    <p className="text-sm text-muted-foreground mb-3 text-center">
                                        {hasMessages ? "What would you like to know more about?" : "Or, you can start with..."}
                                    </p>
                            ...
                            <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                                ...
                                {browserSupportsSpeechRecognition && (
                                    <Button
                                        ...
                                        className={cn("transition-colors", isRecording && "bg-red-500 hover:bg-red-600")}
                                    >
                                ...
                                <Button
                                    type="submit"
                                    size="icon"
                                    disabled={!(inputValue + voiceInputValue).trim()}
        ...
        >
            ...
            <div
                className={cn(
                    ...
                    isAi ? "bg-secondary text-foreground rounded-bl-sm" : "bg-primary text-primary-foreground rounded-br-sm"
                )}
            >
                <p className="leading-relaxed w-full break-words whitespace-pre-wrap">
                    {!isAi ? message.message : shouldAnimate ? <TextGenerateEffect text={content} /> : content}
                </p>
            </div>
