...
const AIChatBubble = () => {
  ...
  const [lastSeenAiTimestamp, setLastSeenAiTimestamp] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    if (!isOpen) return;
    const latestAi = [...messages].filter((m) => m.role === "assistant").at(-1);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("chainlabs:close-ai-chat", close as any);
            ...
            <footer className="p-4 border-t border-border bg-card rounded-b-lg flex-shrink-0">
              ...
              >
                ...
                <Button
                  ...
                  size="icon"
                  disabled={
                    isThinking || !(inputValue + voiceInputValue).trim()
                  }
