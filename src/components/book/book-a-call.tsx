...
const BookCallSection = () => {
  ...
  useEffect(() => {
    if ((meetingCancelled || meetingEnded) && hasBooking && !showScheduler) {
        ...
        {!callUnlocked && (
          <div className="absolute inset-0 z-20 flex items-center justify-center">
            <div className="rounded-2xl bg-background/70 backdrop-blur-md border border-border/50 px-4 py-2 text-sm text-muted-foreground">
              Booking is locked. Complete the required steps to unlock.
            </div>
          </div>
        )}
            ...
            showBookingDetails ? (
                ...
                <div className="rounded-2xl sm:rounded-3xl border border-border/60 bg-gradient-to-br from-surface/80 to-surface-muted/80 backdrop-blur-md p-6 sm:p-8 shadow-lg">
                  ...
                  {!loading && !error && booking && (
                    <>
                      ...
                      <div className="mt-8 flex flex-wrap gap-3">
                        {status === "ACCEPTED" && joinUrl && startDate && (
                          <a
                            href={joinUrl}
                            target="_blank"
                  ...
                  {!loading && !error && status === "PENDING" && (
                    <p className="mt-4 text-xs text-amber-500">
                      Pending approval. You will receive a confirmation email
                      once accepted.
                    </p>
                  )}
                </div>
              ...
              <div className="w-full">
                <div className="mb-6 flex items-center gap-3 justify-center">
                  ...
                  {hasBooking && !meetingCancelled && !meetingEnded && (
                    <button
                      type="button"
                ...
                {meetingCancelled && (
                  <p className="mt-4 text-xs text-muted-foreground text-center">
                    Previous session was cancelled. This will create a new
                    booking.
                  </p>
                )}
                {meetingEnded && (
                  <p className="mt-4 text-xs text-muted-foreground text-center">
                    Previous session ended. Book a follow-up below.
                  </p>
                )}
              </div>
            )
