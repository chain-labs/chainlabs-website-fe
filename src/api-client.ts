...
class ApiClient {
  ...
  async resetSession() {
    try {
      await this.makeAuthenticatedRequest(`${API_BASE_URL}/api/auth/reset`, {
        method: "POST",
      });
    } catch (error) {
  ...
  async submitGoal(input: string) {
    ...
    return {
      ...
      timestamp: new Date(response.assistantMessage.datetime).toISOString(),
    };
  }

  async clarifyGoal(clarification: string) {
    const response: ClarificationResponse = await this.makeAuthenticatedRequest(
      `${API_BASE_URL}/api/clarify`,
      {
        method: "POST",
  ...
  async getPersonalizedContent() {
    const response: PersonalisedResponse = await this.makeAuthenticatedRequest(
