    ...
    const sendMessage = useCallback(
        async (content: string) => {
            const context = {
                ...
                metadata: store.currentContext?.metadata && typeof store.currentContext.metadata === "object"
                    ? { ...store.currentContext.metadata, additionalProp1: store.currentContext.metadata.additionalProp1 ?? null }
    ...
    const sendGoalClarification = useCallback(
        async (clarification: string) => {
            ...
            try {
                ...
                const assistantMessage = {
                    role: "assistant" as const,
                    message: response.headline || "Thank you for the clarification! I've updated your goal and created personalized missions for you.",
                    timestamp: new Date().toISOString(),
                };
                store.addChatMessage(assistantMessage);
                ...
                const errorMessage = {
                    role: "assistant" as const,
                    message: "I'm sorry, I couldn't process your clarification. Please try again.",
                    timestamp: new Date().toISOString(),
                };
                store.addChatMessage(errorMessage);
