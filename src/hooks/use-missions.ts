...
export function useMissions(options: UseMissionsOptions = { autoInit: true }) {
  ...
  const submitAnswer = useCallback(
    async (missionId: string, answer: string): Promise<CompleteMissionResult> => {
      ...
      const doSubmit = async (): Promise<CompleteMissionResult> => {
        try {
          await ensureSession();
          const res = await apiClient.completeMission(missionId, trimmed);
          ...
          if (personalised) {
            ...
            setPersonalised({
              ...
              personalisation: {
                ...
                missions: hasNext ? [...updated, next as Mission] : updated,
              },
            });
          }
