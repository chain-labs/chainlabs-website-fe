...
export const useGlobalStore = create<GlobalState>()(
  persist(
    (set, get) => ({
      ...
      userMessageCount: () => get().messages.filter(m => m.role === "user").length,
      aiMessageCount: () => get().messages.filter(m => m.role === "ai").length,
      shouldShowPersonalized: () => {
        const state = get();
        return state.userMessageCount() >= 2 && state.aiMessageCount() >= 2;
      },
    }),
    {
