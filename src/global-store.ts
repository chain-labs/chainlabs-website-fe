...
interface SessionActions {
  ...
  updateMissionStatus: (missionId: string, status: "pending" | "completed") => void;
  ...
  updateProgress: (progress: { points_total: number; missions: MissionStatus[]; call_unlocked: boolean }) => void;
...
export const useGlobalStore = create<SessionState & SessionActions>()(
  persist(
    (set, get) => ({
      ...
      setShowPersonalized: (showPersonalized) => set({ showPersonalized }),
      ...
      hydrateFromSession: (sessionData) =>
        set({
          ...
          missions: sessionData.missions?.map((m: any) => ({
            id: m.id,
            title: m.title || m.id,
            points: m.points,
            status: m.status
