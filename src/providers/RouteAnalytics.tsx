      ...
      if (typeof window.gtag === "function") {
        ...
        window.gtag("event", "page_view", {
          ...
          custom_dimensions: {
            ...
            missions_completed: store.missions.filter(
        ...
        if (store.personalised?.sid) {
          window.gtag(
            "config",
            process.env.NEXT_PUBLIC_GA_MEASUREMENT_ID || "G-2TXCJ3VJ1B",
            {
          ...
          user_data: {
            ...
            missions_progress_percent:
              store.missions.length > 0
                ? Math.round(
                    (store.missions.filter((m) => m.status === "completed")
                      .length /
  ...
  useEffect(() => {
    ...
    const trackPageSpecificEvents = () => {
      try {
        ...
        console.error(
          "[Analytics] Error tracking page-specific events:",
          error,
        );
      }
    };
